...
def split_dataset_by_questions(dataset, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15):
    """
    Split dataset ensuring NO question appears in multiple splits
    """
    ...
    if overlap_train_val or overlap_train_test or overlap_val_test:
        logger.error("OVERLAP DETECTED! This shouldn't happen with proper splitting.")
    else:
        logger.info("No overlap detected - splits are clean!")
    
    return train_data, val_data, test_data

def create_supervised_simcse_pairs(data, output_file, max_pairs=50000):
    """
    Create SUPERVISED SimCSE training pairs (WITH labels) - both positive and negative pairs
    """
    logger.info(f"Creating SUPERVISED SimCSE pairs from {len(data)} samples...")
    
    pairs = []
    
    # First, collect all contexts for negative sampling
    all_contexts = []
    all_answers = []
    ...
    for item in data:
        ...
        if len(all_answers) > 1:
            # Get wrong answers (not belonging to this question)
    ...
    for i, item in enumerate(data):
        ...
        if question and len(all_contexts) > i + 10:
            ...
            for neg_idx in negative_indices:
                ...
                if len(pairs) % 100 == 0:  # Don't create too many negatives
                    break
...
def verify_no_overlap(train_file, val_file, test_file):
    """
    Verify that there's no question overlap between files
    """
    ...
    if train_test_overlap:
        logger.error("CRITICAL: Train-Test overlap detected!")
        logger.error("Sample overlapping questions:")
        for q in list(train_test_overlap)[:3]:
            logger.error(f"  '{q[:100]}...'")
        return False
    else:
        logger.info("? SUCCESS: No train-test overlap!")
        return True

def main():
    """Main preprocessing function with SUPERVISED SimCSE training data"""
    ...
    try:
        ...
        # Split dataset by questions (NO OVERLAP)
        ...
        # Create SUPERVISED training pairs (WITH labels - positive AND negative)
        ...
        if is_clean:
            logger.info("? SUCCESS: Clean SUPERVISED SimCSE dataset created!")
            logger.info("This should give you much better contrastive loss (<0.02) and MIRAGE performance!")
        else:
            logger.error("? FAILED: Data leakage detected. Do not use these files!")
