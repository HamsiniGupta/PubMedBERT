...
def compute_similarity_scores(model, sentences1, sentences2):
    """Compute cosine similarity scores for sentence pairs"""
...
def main():
    ...
    print(f"\nSummary:")
    print(f"Your model achieved {metrics['accuracy']:.1%} accuracy")
    print(f"Correlation with labels: {metrics['correlation']:.4f}")
    
    if metrics['correlation'] > 0.5:
        print("Strong correlation - model learned meaningful representations!")
    elif metrics['correlation'] > 0.3:
        print("Moderate correlation - model is working but could improve")
    else:
