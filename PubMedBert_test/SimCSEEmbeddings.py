...
class SimCSEEmbeddings:
    ...
    def _load_model_with_config_fix(self):
        """Load the model by temporarily fixing the config"""
        ...
        # Check if we need to fix the config
    ...
    def _load_simcse_components(self):
        """Load SimCSE components with proper error handling"""
        ...
        if os.path.exists(simcse_components_path):
            try:
                ...
                if isinstance(simcse_components, dict):
                    # Use the actual keys from your file structure
                    self.config = {
                    ...
                    # Your model uses identity mapping (no MLP projection)
    ...
    def _use_defaults(self):
        """Use default configuration when simcse_components.pt is not available"""
        self.config = {
            'pooler_type': 'cls',
            'temp': 0.05,
            'model_type': 'simcse'
    ...
    def encode(self, texts: Union[str, List[str]], batch_size: int = 32, max_length: int = 128) -> np.ndarray:
        """
        Encode texts using the trained SimCSE model
        """
    ...
    def embed_query(self, text: str) -> List[float]:
        """
        Embed a single query (required by your RAG pipeline)
        """
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        """
        Embed multiple documents (might be needed by your RAG pipeline)
        """
...
if __name__ == "__main__":
    # Test with your latest checkpoint
    model_path = "output/pubmedqa-supervised-simcse"  # or specific checkpoint
