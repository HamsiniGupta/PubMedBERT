...
def evaluate_simcse_embeddings(model_path, eval_file="data/pubmedqa_val_clean.csv"):
    ...
    # ROC-AUC (how well similarity scores separate relevant vs irrelevant)
    ...
    if auc_score > 0.8:
        print("EXCELLENT: PubMedBert model shows strong semantic understanding")
    elif auc_score > 0.7:
        print("GOOD: PubMedBert model captures semantic similarity well")
    elif auc_score > 0.6:
        print("FAIR: PubMedBert model shows some semantic understanding")
    else:
        print("POOR: PubMedBert model may need more training or data")
    
    print(f"Key metrics:")
    print(f"- ROC-AUC: {auc_score:.4f}")
    print(f"- Best F1: {best_f1:.4f}")
    print(f"- Relevant vs Irrelevant gap: {pos_similarities.mean() - neg_similarities.mean():.4f}")
    # At the end of your evaluate_simcse_embeddings function, before the return statement:
...
def generate_roc_analysis(similarities, labels, auc_score, best_threshold):
    ...
    # Performance interpretation removed - clean ROC curve without labels
    ...
    Clinical Relevance:
    High recall is important for medical
    applications to avoid missing relevant
    medical information.
    """
    ...
    print(f"   ROC-AUC = {auc_score:.4f}")
    if auc_score > 0.9:
        print("   ?? EXCELLENT: Model has outstanding discriminative ability")
    elif auc_score > 0.8:
        print("   ?? VERY GOOD: Model shows strong discriminative ability")
    elif auc_score > 0.7:
        print("   ?? GOOD: Model has decent discriminative ability")
    elif auc_score > 0.6:
        print("   ?? FAIR: Model shows some discriminative ability")
    else:
        print("   ?? POOR: Model has limited discriminative ability")
    
    print(f"\n?? Precision-Recall AUC = {pr_auc:.4f}")
    print(f"   This is especially important for imbalanced datasets")
def create_horizontal_confusion_matrix(similarities, labels, threshold, save_path="plots/confusion_matrix_horizontal.png"):
    """Create a horizontal confusion matrix with green colormap"""
    from sklearn.metrics import confusion_matrix
    import seaborn as sns
    import matplotlib.pyplot as plt
    ...
    plt.figure(figsize=(10, 6))  # Wider than tall for horizontal layout
    
    # Create heatmap with green colormap
    sns.heatmap(cm, annot=True, fmt='d', cmap='Greens',  # Changed from 'Blues' to 'Greens'
                xticklabels=['Irrelevant', 'Relevant'],
                yticklabels=['Irrelevant', 'Relevant'],
                cbar_kws={'shrink': 0.8},
                annot_kws={'size': 22})
...
def create_similarity_plots(pos_similarities, neg_similarities, similarities, labels):
    """
    Create the original similarity distribution plots
    """
